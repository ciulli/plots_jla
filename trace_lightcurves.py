...
def read_jla_lcparams(list_file='/sps/lsst/dev/ciulli/JLA_lightcurves/jla_likelihood_v4/data/jla_lcparams.txt', zmax=False):
    params=read_list(list_file, SkipRows=1, delimiteur=' ')
    dic_param={}
    for i in range(len(params[0])):
        if zmax:
            if float(params[1][i]) >= zmax:
                continue
        dic_param[params[0][i]] = {}
        dic_param[params[0][i]]['ra'] = float(params[18][i])
        dic_param[params[0][i]]['dec'] = float(params[19][i])
        dic_param[params[0][i]]['zcmb'] = float(params[1][i])
        dic_param[params[0][i]]['zhelio'] = float(params[2][i])
...
if __name__=="__main__":
    ...
    dicparam=read_jla_lcparams(list_file='/sps/lsst/dev/ciulli/JLA_lightcurves/jla_likelihood_v4/data/jla_lcparams.txt')
    print ''
    save_pkl(dicparam,'dic_param.pkl')
    dicparam1=read_jla_lcparams(list_file='/sps/lsst/dev/ciulli/JLA_lightcurves/jla_likelihood_v4/data/jla_lcparams.txt', zmax=0.1)
